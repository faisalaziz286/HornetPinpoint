...
angular.module('hpinpoint', ['ionic', 'ngCordova', 'http-auth-interceptor',
    ...
    .run(["$ionicPlatform", "$ionicLoading", "$rootScope", "$ionicPopup", "Hornet", function ($ionicPlatform, $ionicLoading, $rootScope, $ionicPopup, Hornet) {
...
angular.module('hpinpoint.controllers', [])
    .controller('NearCtrl', ["$scope", "Hornet", "$ionicPopup", "$ionicLoading", "$state", function ($scope, Hornet, $ionicPopup, $ionicLoading, $state) {
    ...
    .controller('SearchCtrl', ["$scope", "Hornet", "$ionicPopup", "$ionicLoading", "$state", function ($scope, Hornet, $ionicPopup, $ionicLoading, $state) {
    // With the new view caching in Ionic, Controllers are only called
    // when they are recreated or on app start, instead of every page change.
    // To listen for when this page is active (for example, to refresh data),
    // listen for the $ionicView.enter event:
    ...
    .controller('LocateCtrl', ["$scope", "Hornet", "$stateParams", "$ionicLoading", "$ionicPopup", function ($scope, Hornet, $stateParams, $ionicLoading, $ionicPopup) {
...
angular.module('hpinpoint.services', ['ionic'])
    .factory('Hornet', ["$http", "$q", "CurrentLocation", "$rootScope", "authService", function ($http, $q, CurrentLocation, $rootScope, authService) {
    ...
    function geodist(p, ref) {
        ...
        ret.lng = ref.lng + deg(Math.atan2(bsin * Math.sin(rdist) * Math.cos(rlat), Math.cos(rdist) - Math.sin(rlat) * Math.sin(lat)));
        if (lat < -Math.PI)
    ...
    service.searchUser = function (string, page) {
        ...
        return $http.get(baseURL + '/members/search?page=' + page + '&per_page=50&username=' + string, { headers: http_headers });
    };
    ...
    service.getDistance = function (userId, loc) {
        ...
        $http.get(baseURL + "/members/search?page=1&per_page=50&username=@" + userId, { headers: h })
            .then(function (response) {
    ...
    service.pinpoint = function (userID) {
        ...
        function worker() {
            ...
            service.getDistance(userID, loc).then(function (d) {
    ...
    return service;
}]).factory('CurrentLocation', ["$cordovaGeolocation", "$rootScope", function ($cordovaGeolocation, $rootScope) {
